...
void dump(void)
{
  ...
  while (p != NULL) {
    up.part = *p;
    fwrite(&up.dump, sizeof(up.dump), 1, fp);
    p = p->next;
  }
  fclose(fp);
}
...
void restore(void)
{
  ...
  union u_part up = {.part.next = NULL};
  ...
  /* first, wipe the existing inventory to avoid memory leak */
  while (*p != NULL) {
    struct part *prev = *p;
    p = &(*p)->next;
    free(prev);
  ...
  p = &inventory;
  while (fread(&up.dump, sizeof(up.dump), 1, fp) == 1) {
    if ((new_part = malloc(sizeof(struct part))) == NULL) {
      printf("Error in restore: malloc cannot allocate memory\n");
      exit(EXIT_FAILURE);
    }
    *new_part = up.part;
    *p = new_part;
